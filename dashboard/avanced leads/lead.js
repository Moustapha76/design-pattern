 ...
 * https://in.linkedin.com/in/yogeshpaghdal
 * @license MIT
 */
(function (window, angular, undefined) {
    ...
    function mdTable() {
      ...
      function compile(tElement, tAttrs) {
        ...
        //var containerWidth = angular.element(document.querySelector('#tablewidth')).clientWidth;	
        //  angular.element(document.querySelector("#leftnavpanel")).removeClass("showleftbar");
    ...
    angular.module('md.data.table').directive('resize', resize);
    function resize($document, $window) {
      return {
        link: function(scope, elem, attrs) {
          var TotalFixWidth = window.innerWidth;
          var Fieldwidth = 100;	
           angular.element($window).bind('load', function() {
                 onResize();
           });
             function onResize() {
                var tablewidth = (elem[0].offsetWidth); /* final width of table */
                var totalwidth = window.innerWidth; /* final window of table */
                    if(tablewidth >= totalwidth){
                      angular.element(document.querySelectorAll("tr .plusbuttonicon")).css("display","block");
                      var TableThLength = angular.element(document.querySelectorAll("tr th.Present")).length;
                      angular.element(document.querySelector("thead tr th.thaa"+ TableThLength)).removeClass("Present");
                      angular.element(document.querySelector("thead tr th.thaa"+ TableThLength)).addClass("overhide");
                      angular.element(document.querySelectorAll("tbody tr td.thaa"+ TableThLength)).addClass("overhide");  
                      Fieldwidth = (elem[0].offsetWidth) + 50;
                        for(var i=1; i< TableThLength; i++){  onResize();  break;  }
                        angular.element(document.querySelectorAll("tbody tr.secondary")).empty();
                    }else{
                          if(TotalFixWidth <= totalwidth){
                           if(Fieldwidth < tablewidth){ 
                            Fieldwidth = tablewidth + 70;
                            
                             var TableThLengthY = (angular.element(document.querySelectorAll("tr th.Present")).length);
                             angular.element(document.querySelector("tr th.thaa"+ (TableThLengthY + 1))).addClass("Present");
                             angular.element(document.querySelector("tr th.thaa"+ (TableThLengthY + 1))).removeClass("overhide");
                             angular.element(document.querySelectorAll("tr td.thaa"+ (TableThLengthY + 1))).removeClass("overhide");
                             angular.element(document.querySelectorAll("tbody tr.secondary")).empty();
                                 
                              var TableThLength = (angular.element(document.querySelectorAll("tr th")).length) - 3;
                              console.log(TableThLength+ " == " + TableThLengthY);
                              if(TableThLengthY === (TableThLength -1)){
                                angular.element(document.querySelector("tr th")).removeClass("overhide");
                                angular.element(document.querySelectorAll("tr td")).removeClass("overhide");
                                  angular.element(document.querySelectorAll("tr .plusbuttonicon")).css("display","none");
                              }
                           }
                         } 
                         TotalFixWidth = window.innerWidth;	
                     }	
                }
            angular.element($window).on('resize', onResize);
          
             
            /* onclick event to add attributes */  
            var movedata = function(event) {
                  var expanttr = event.target.offsetParent.id;
                 if(event.target.id === 'uparrow'){
                    angular.element(document.querySelectorAll("tbody tr.childid"+ expanttr)).empty();
                    angular.element(document.querySelector("button.downarrow-"+ expanttr)).css("display","block");
                    angular.element(document.querySelector("button.uparrow-"+ expanttr)).css("display","none");
                 }
                 if(event.target.id === 'downarrow'){
                    angular.element(document.querySelector("button.downarrow-"+ expanttr)).css("display","none");
                    angular.element(document.querySelector("button.uparrow-"+ expanttr)).css("display","block");
                    angular.element(document.querySelectorAll("tbody tr.childid"+ expanttr)).prepend(angular.element(document.querySelectorAll("tbody tr.trid"+ expanttr +" td.overhide")).clone());
                 }
            };
            elem.bind('click', movedata);
        }
      }
    }
